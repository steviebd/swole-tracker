...
**Task 0.3.1: Create src/routes/__root.tsx**
```typescript
...
export async function getServerContext(): Promise<ServerContext> {
  ...
  // E2E Test Bypass
  const isE2ETest = env.E2E_TESTING === 'true' || 
...
export async function requireAuth(): Promise<ServerContext & { user: { id: string } }> {
...
- [ ] `bun run lint` passes  
- [ ] `bun run build` creates valid worker
- [ ] Dev server runs at localhost:8787
- [ ] Next.js and tRPC dependencies fully removed
- [ ] Playwright configured for new server port
...
export const getTemplates = createServerFn({ method: 'GET' })
  ...
  .validator(z.object({
    ...
    sort: z.enum(['recent', 'lastUsed', 'mostUsed', 'name']).default('recent'),
  }).optional())
  .handler(async ({ context, data }) => {
    ...
    const queryBuilder = db
      ...
      .leftJoin(workoutSessions, eq(workoutTemplates.id, workoutSessions.templateId))
    ...
    const statsByTemplate = new Map(
      results.map((row) => [
        row.template.id,
        { lastUsed: row.lastUsed ?? null, totalSessions: Number(row.totalSessions ?? 0) },
      ])
    )
...
export const templatesQueryOptions = (params?: { search?: string; sort?: string }) =>
  queryOptions({
