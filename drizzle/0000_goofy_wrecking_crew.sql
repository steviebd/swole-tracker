...
--> statement-breakpoint
CREATE INDEX `daily_joke_user_id_idx` ON `daily_joke` (`user_id`);--> statement-breakpoint
CREATE INDEX `daily_joke_created_at_idx` ON `daily_joke` (`createdAt`);--> statement-breakpoint
CREATE INDEX `daily_joke_user_date_idx` ON `daily_joke` (`user_id`,`createdAt`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `session_debrief_user_idx` ON `session_debrief` (`user_id`);--> statement-breakpoint
CREATE INDEX `session_debrief_session_idx` ON `session_debrief` (`sessionId`);--> statement-breakpoint
CREATE INDEX `session_debrief_created_idx` ON `session_debrief` (`createdAt`);--> statement-breakpoint
CREATE INDEX `session_debrief_user_created_idx` ON `session_debrief` (`user_id`,`createdAt`);--> statement-breakpoint
CREATE TABLE `session_exercise` (
	...
	`one_rm_estimate` real GENERATED ALWAYS AS (CASE WHEN weight IS NOT NULL AND reps IS NOT NULL AND weight > 0 AND reps > 0 THEN weight * (1 + reps / 30.0) ELSE NULL END) VIRTUAL,
	`volume_load` real GENERATED ALWAYS AS (CASE WHEN weight IS NOT NULL AND reps IS NOT NULL AND sets IS NOT NULL AND weight > 0 AND reps > 0 AND sets > 0 THEN sets * reps * weight ELSE NULL END) VIRTUAL,
	`createdAt` text DEFAULT (datetime('now')) NOT NULL,
	FOREIGN KEY (`sessionId`) REFERENCES `workout_session`(`id`) ON UPDATE no action ON DELETE cascade,
...
CREATE INDEX `template_exercise_order_idx` ON `template_exercise` (`templateId`,`orderIndex`);--> statement-breakpoint
