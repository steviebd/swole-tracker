...
export function calculateReadiness(whoop: WhoopMetrics): {
  ...
  const s =
    whoop.sleep_performance != null ? whoop.sleep_performance / 100 : 0.5;
...
> {
  ...
  await whereInChunks(exerciseNames, async (nameChunk) => {
    const chunkSessions = await db.query.workoutSessions.findMany({
      where: and(
        eq(db.schema.workoutSessions.user_id, userId),
        excludeSessionId
          ? ne(db.schema.workoutSessions.id, excludeSessionId)
          : undefined,
      ),
      orderBy: [desc(db.schema.workoutSessions.workoutDate)],
      with: {
        exercises: {
          where: (exercises: any) => inArray(exercises.exerciseName, nameChunk),
  ...
  const recentSessionsWithExercises = Array.from(sessionsById.values())
    .sort(
      (a, b) =>
  ...
  for (const session of recentSessionsWithExercises) {
    for (const exercise of session.exercises) {
      ...
      const sessionsForExercise = exerciseSessionMap.get(
        exercise.exerciseName,
      )!;

      let sessionGroup = sessionsForExercise.find(
      ...
      sessionGroup.sets.push({
        ...
        volume:
          exercise.weight && exercise.reps
            ? parseFloat(exercise.weight) * exercise.reps
            : null,
      });
    }
  }

  return Array.from(exerciseSessionMap.entries()).map(
      ...
      switch (progressionType) {
        ...
        case "percentage":
          const percentageIncrement =
            (userPreferences.percentageIncrement || 2.5) / 100;
        ...
        default:
          if (plateauDetected && readiness < 0.7) {
            ...
            rationale =
              "Deload recommended: plateau detected with poor readiness";
          } else if (plateauDetected) {
            progressionFactor = 1.025;
            rationale =
              "Light progression despite plateau (good readiness allows push)";
          } else {
            progressionFactor =
              readiness > 0.7 ? 1.05 : readiness > 0.5 ? 1.0 : 0.975;
            rationale = `Adaptive progression based on ${readiness > 0.7 ? "excellent" : readiness > 0.5 ? "good" : "low"} readiness`;
          }

          const progressionModel =
            userPreferences.progressionModel ||
            (progressionFactor === 1.0 ? "reps" : "weight");
          ...
          if (progressionModel === "weight") {
            ...
            suggestions.push({
              ...
              rationale:
                "Volume progression: add 1 rep while maintaining weight",
              plateauDetected,
            });
          }
          break;
