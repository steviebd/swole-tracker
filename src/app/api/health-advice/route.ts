...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        if (latestRecovery || latestSleep) {
          ...
          realWhoopData = {
            recovery_score:
              ...
              realWhoopData.recovery_score ||
              defaultWhoopData.recovery_score,
            sleep_performance:
            ...
            rhr_now_bpm:
              ...
              realWhoopData.rhr_now_bpm ||
              defaultWhoopData.rhr_now_bpm,
            rhr_baseline_bpm:
              latestRecovery?.resting_heart_rate_baseline ||
              realWhoopData.rhr_baseline_bpm ||
              defaultWhoopData.rhr_baseline_bpm,
            yesterday_strain:
              realWhoopData.yesterday_strain ||
              defaultWhoopData.yesterday_strain, // Could be fetched from cycles table if needed
          };
    ...
    if (hasManualWellness && validatedInput.manual_wellness) {
      ...
      rho = Math.max(
        0,
        Math.min(1, 0.6 * energyComponent + 0.4 * sleepComponent),
      );
      flags = [];
      ...
      if (manualWellness.notes?.toLowerCase().includes("stress"))
        flags.push("stress_noted");
      if (manualWellness.notes?.toLowerCase().includes("sick"))
        flags.push("illness_noted");
    ...
    const delta = calculateOverloadMultiplier(
      rho,
      validatedInput.user_profile.experience_level,
    );
    ...
    if (rho < 0.35) {
      return NextResponse.json({
        ...
        readiness: {
          ...
          flags: [...flags, "unsafe_readiness"],
        },
        per_exercise: [],
        session_predicted_chance: 0.3,
        summary:
          "Your recovery metrics suggest taking it easy today. Stick to your planned loads.",
    ...
    if (exerciseHistory.length > 0) {
      try {
        progressionSuggestions = calculateProgressionSuggestions(
          exerciseHistory,
          rho,
          (userProgressionPrefs?.progression_type as
            | "linear"
            | "percentage"
            | "adaptive") || "adaptive",
          {
    ...
    if (!hasKey || templateExercises.length === 0) {
      ...
      const perExerciseAdvice = exercisesToUse.map(
        (exercise: {
          exerciseName?: string;
          exercise_id?: string;
          id?: string;
          ...
          const exerciseHist = exerciseHistory.find(
            (hist) =>
              ...
              hist.exerciseName === exercise.exercise_id,
          );

          const progressionSuggestion = progressionSuggestions.find(
            (prog) =>
              prog.exerciseName === exercise.exerciseName ||
          ...
          if (
            ...
            const weightSuggestion = progressionSuggestion.suggestions.find(
              (s) => s.type === "weight",
            );
            const repsSuggestion = progressionSuggestion.suggestions.find(
              (s) => s.type === "reps",
            );
          ...
          const bestVolume =
            exerciseHist?.sessions[0]?.sets.reduce(
              (total: number, set: WorkoutSet) => total + (set.volume || 0),
              0,
            ) || 0;
          ...
          return {
            ...
            predicted_chance_to_beat_best:
              rho > 0.7 ? 0.8 : rho > 0.5 ? 0.6 : 0.4,
      ...
      const totalSets = perExerciseAdvice.reduce(
        (sum: number, ex: { sets: { length: number }[] }) =>
          sum + ex.sets.length,
        0,
      );
      const avgRestTime = rho > 0.7 ? 120 : rho > 0.5 ? 150 : 180;
      const estimatedDuration = Math.round(
        (totalSets * avgRestTime + totalSets * 60) / 60,
      ); // Rest + exercise time

      // Check for plateau warnings
      const plateauCount = progressionSuggestions.filter(
        (p) => p.plateauDetected,
      ).length;
      const plateauWarnings =
        plateauCount > 0
          ? [
              `Plateau detected in ${plateauCount} exercise${plateauCount > 1 ? "s" : ""} - consider deload or variation`,
            ]
          : [];
      ...
      switch (progressionType) {
        ...
        default:
          progressionSummary =
            "Using adaptive progression based on readiness and performance";
          break;
      }
      ...
      return NextResponse.json({
        ...
        warnings: [
          ...(hasKey
            ? []
            : [
                "AI Gateway not configured - using enhanced fallback calculations",
              ]),
          ...(hasManualWellness
            ? ["Recommendations based on manual wellness input"]
            : []),
          ...plateauWarnings,
        ],
        recovery_recommendations: {
          recommended_rest_between_sets: `${Math.round(avgRestTime / 60)} minutes for strength exercises`,
          recommended_rest_between_sessions:
            rho > 0.7
              ? "24-48 hours"
              : rho > 0.5
                ? "48-72 hours"
      ...
      exercise_linking: await (async () => {
        try {
          ...
          const linksByTemplateId = new Map<
            number,
            (typeof exerciseLinksForTemplate)[0]
          >();
          const linksByMasterId = new Map<
            number,
            (typeof exerciseLinksForTemplate)[0][]
          >();
          ...
          return templateExercises.map((ex) => {
            ...
            if (exerciseLink) {
              const linkedExercises =
                linksByMasterId.get(exerciseLink.masterExerciseId) || [];
