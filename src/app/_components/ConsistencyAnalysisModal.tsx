...
export function ConsistencyAnalysisModal({
  ...
  const { data: consistencyData, isLoading } =
  ...
  const { data: workoutDatesData, isLoading: workoutDatesLoading } =
    api.progress.getWorkoutDates.useQuery({ timeRange }, { enabled: isOpen });
  ...
  const calculateDetailedStats = () => {
    ...
    const mostActiveDay = (
      Object.entries(workoutsByDayOfWeek) as [string, number][]
    ).sort(([, a], [, b]) => b - a)[0];
    ...
    const targetProgress = Math.min(
      (actualWorkouts / targetWorkouts) * 100,
      100,
    );
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                >
                  <h5
                    className={`mb-3 text-sm font-semibold ${isDark ? "text-gray-200" : "text-gray-800"}`}
                  >
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      const count =
                        detailedStats.workoutsByDayOfWeek[index] || 0;
                      const maxCount = Math.max(
                        ...(Object.values(
                          detailedStats.workoutsByDayOfWeek,
                        ) as number[]),
                      );
                      const percentage =
                      ...
                      return (
                        <div key={day} className="flex items-center space-x-3">
                          <span
                            className={`w-12 text-xs ${isDark ? "text-gray-300" : "text-gray-700"}`}
                          >
                            {day.slice(0, 3)}
                          </span>
                          <div
                            className={`h-4 flex-1 rounded-full ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
                          >
                            <div
                          ...
                          <span
                            className={`w-6 text-right text-xs ${isDark ? "text-gray-300" : "text-gray-700"}`}
                          >
                            {count}
                          </span>
                        </div>
                  ...
                  <p
                    className={`mt-3 text-xs ${isDark ? "text-gray-400" : "text-gray-600"}`}
                  >
                    Most active day:{" "}
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-lg font-bold ${isDark ? "text-background" : "text-foreground"}`}
                      >
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {calendars.map((calendar, calIndex) => (
                  <div
                    key={calIndex}
                    className={`rounded-lg p-3 ${isDark ? "bg-gray-800" : "bg-gray-50"}`}
                  >
                    <h5
                      className={`mb-2 text-center text-xs font-semibold ${isDark ? "text-gray-200" : "text-gray-800"}`}
                    >
                      {calendar.label}
                    </h5>
                    ...
                    <div
                      className={`grid ${timeRange === "week" ? "grid-cols-7" : "grid-cols-7"} gap-1`}
                    >
                      {calendar.days.map((date, index) => (
                        <div key={index} className="aspect-square">
                          {date ? (
                            <div
                              className={`flex h-full w-full items-center justify-center rounded text-xs transition-all ${
                                isToday(date)
              ...
              <div className="mt-4 flex items-center justify-center space-x-6 text-xs">
                ...
                <div className="flex items-center space-x-1">
                  <div
                    className={`h-3 w-3 rounded border border-blue-500 ${isDark ? "bg-blue-900" : "bg-blue-100"}`}
                  ></div>
                  <span className={isDark ? "text-gray-400" : "text-gray-600"}>
                    Today
                  </span>
                ...
                <div className="flex items-center space-x-1">
                  <div
                    className={`h-3 w-3 rounded ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
                  ></div>
                  <span className={isDark ? "text-gray-400" : "text-gray-600"}>
                    Rest Day
                  </span>
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p
                        className={`mt-1 text-xs ${isDark ? "text-gray-400" : "text-gray-600"}`}
                      >
                        Consider scheduling important workouts on your most
                        consistent day.
                      </p>
                    </div>
                  </div>
