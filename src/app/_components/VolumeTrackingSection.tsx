...
export function VolumeTrackingSection() {
  ...
  const { data: exerciseList } = api.progress.getExerciseList.useQuery({
    limit: 100,
    offset: 0,
  });
        ...
        <>
          ...
          <div className="mb-6">
            ...
            <div className="relative h-64 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4">
              <svg className="h-full w-full" viewBox="0 0 400 200">
                ...
                {chartData.length > 1 && (
                  <polygon
                    points={`20,180 ${chartData
                      .map((point, index) => {
                        const x =
                          (index / Math.max(1, chartData.length - 1)) * 360 +
                          20;
                        const y =
                          180 -
                          ((point[selectedMetric] - minValue) / range) * 160;
                        return `${x},${y}`;
                      })
                      .join(
                        " ",
                      )} ${20 + ((chartData.length - 1) / Math.max(1, chartData.length - 1)) * 360},180`}
                ...
                {chartData.map((point, _index) => {
                  const x =
                    (_index / Math.max(1, chartData.length - 1)) * 360 + 20;
                  const y =
                    180 - ((point[selectedMetric] - minValue) / range) * 160;
          ...
          <div className="mb-6">
            ...
            {volumeByExerciseLoading ? (
              ...
              <div className="space-y-4">
                {/* Donut Chart */}
                <div className="relative h-80 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4">
