...
export function StrengthAnalysisModal({
  ...
  const { data: strengthResponse, isLoading } =
    api.progress.getStrengthProgression.useQuery(
      ...
      },
      {
        enabled:
          isOpen &&
          (exerciseName.length > 0 || typeof templateExerciseId === "number"),
      },
    );
  ...
  const calculateStats = () => {
    if (!strengthResponse?.data || strengthResponse.data.length === 0)
      return null;

    const data = strengthResponse.data;
    const weights = data.map((d) => d.weight);
    const oneRMs = data.map((d) => d.oneRMEstimate);
    ...
    const totalProgress =
      firstEntry && lastEntry ? lastEntry.weight - firstEntry.weight : 0;
    const progressPercentage = firstEntry
      ? (totalProgress / firstEntry.weight) * 100
      : 0;

    // Volume stats
    const totalVolume = data.reduce(
      (sum, session) => sum + session.weight * session.reps * session.sets,
      0,
    );
    const avgVolumePerSession = totalVolume / data.length;

    // Session stats
    const totalSessions = data.length;
    const totalSets = data.reduce((sum, session) => sum + session.sets, 0);
    const totalReps = data.reduce(
      (sum, session) => sum + session.reps * session.sets,
      0,
    );

    const sessionFrequency =
      data.length > 1
        ? new Date(data[0]!.workoutDate).getTime() -
          new Date(data[data.length - 1]!.workoutDate).getTime()
        : 0;

    // Consistency analysis
    const dateRange = sessionFrequency;
    const daysSpanned = Math.ceil(dateRange / (1000 * 60 * 60 * 24));
    const sessionsPerWeek =
      daysSpanned > 0 ? (totalSessions / daysSpanned) * 7 : 0;
  ...
  return (
    <div className="bg-background bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        className={`max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl ${cardClass} p-6`}
      >
        ...
        {isLoading ? (
          ...
          <>
                ...
                >
                  <div className="mb-1 flex items-center space-x-2">
                    <span className="text-xl">
                      {getProgressIcon(stats.progressPercentage)}
                    </span>
                    <h5
                      className={`text-xs font-medium ${isDark ? "text-gray-400" : "text-gray-600"}`}
                    >
                  ...
                  <p
                    className={`text-2xl font-bold ${getProgressColor(stats.progressPercentage)}`}
                  >
                    {stats.totalProgress > 0 ? "+" : ""}
                    {stats.totalProgress}kg
                  </p>
                  <p
            ...
            <div>
              ...
              <div
                ...
                } mobile-table-container max-h-64 overflow-hidden overflow-y-auto`}
              >
                <div className="mobile-table">
                  <div
                    className={`sticky top-0 grid grid-cols-6 gap-2 border-b px-2 py-3 text-xs font-medium sm:gap-4 sm:px-4 ${
                      isDark
          ...
          <div className="py-12 text-center">
            ...
            >
              Complete some workouts with {exerciseName} to see detailed
              analysis.
            </p>
          </div>
        )}
