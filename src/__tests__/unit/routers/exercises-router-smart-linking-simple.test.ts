...
describe("exercisesRouter - Smart Linking Tests", () => {
  ...
  describe("getSmartLinkingSuggestions", () => {
    it("should return suggestions with exact matches", async () => {
      ...
      mockCtx.db.queueSelectResult(mockMasterExercises);
      ...
      expect(result.summary.autoLinkCount).toBeGreaterThanOrEqual(0);
      expect(result.summary.needReviewCount).toBeGreaterThanOrEqual(0);
      expect(result.summary.createNewCount).toBeGreaterThanOrEqual(0);
    });
    ...
    it("should handle database errors gracefully", async () => {
      mockCtx.db.select.mockImplementation(() => {
    ...
    it("should limit matches to top 5 per exercise", async () => {
      ...
      expect(result.suggestions[0]?.matches.length).toBeLessThanOrEqual(5);
    });
    ...
    it("should handle custom similarity threshold", async () => {
      ...
      expect(result.suggestions[0]?.matches.length).toBeGreaterThanOrEqual(0);
    });
