...
describe('offline-storage: setupOfflinePersistence and helpers', () => {
  ...
  it('setupOfflinePersistence calls persistQueryClient with expected options in browser', async () => {
    ...
    vi.mock('@tanstack/query-sync-storage-persister', async () => {
      ...
      return await vi.importActual<any>('@tanstack/query-sync-storage-persister');
    });
    ...
    const mocked = await vi.importMock<any>('@tanstack/react-query-persist-client');
    const persistSpy = mocked.persistQueryClient as unknown as ReturnType<typeof vi.fn>;
