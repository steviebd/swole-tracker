...
describe('rate-limit (db behavior simulated)', () => {
  ...
  it('creates new rate limit record if none exists and allows request', async () => {
    ...
    const wherePromise = new Promise<any[]>((res) => { whereResolve = res; });
    ...
    const res = await mod.checkRateLimit('userA', '/endpoint', 5, 1000 * 60 * 60);
  ...
  it('denies when limit already reached', async () => {
    ...
    const res = await mod.checkRateLimit('userB', '/endpoint', 3, 1000 * 60 * 60);
  ...
  it('increments request count when under limit', async () => {
    ...
    const res = await mod.checkRateLimit('userC', '/endpoint', 3, 1000 * 60 * 60);
    expect(res.allowed).toBe(true);
