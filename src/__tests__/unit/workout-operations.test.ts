...
describe('workout-operations client and server wrappers', () => {
  ...
  function makeSupabaseClient() {
    ...
    const chain: any = {
      ...
      then: (onFulfilled: any, _onRejected?: any) => Promise.resolve(finalResult).then(onFulfilled),
      // Ensure await works by providing a catch as well
      catch: (onRejected: any) => Promise.resolve(finalResult).catch(onRejected),
    };
  ...
  it('WorkoutOperationsClient.getWorkoutTemplates queries by user and orders', async () => {
    ...
    const { WorkoutOperationsClient } = await import('~/lib/workout-operations');

    const inst = new WorkoutOperationsClient({ getToken: async () => 't' });
    const res = await inst.getWorkoutTemplates('user_1');

    expect(clientMock.from).toHaveBeenCalledWith('swole-tracker_workout_template');
    expect(clientMock.chain.eq).toHaveBeenCalledWith('user_id', 'user_1');
    expect(clientMock.chain.order).toHaveBeenCalledWith('createdAt', { ascending: false });
  ...
  it('WorkoutOperationsClient.createWorkoutTemplate inserts and returns single', async () => {
    ...
    expect(clientMock.from).toHaveBeenCalledWith('swole-tracker_workout_template');
    expect(clientMock.chain.insert).toHaveBeenCalledWith({ name: 'Leg Day', user_id: 'user_2' });
  ...
  it('WorkoutOperationsClient.getRecentWorkouts limits, orders, and returns fields', async () => {
    ...
    clientMock.setResult({
      data: [{ id: 1, templateId: 2, workoutDate: '2024-01-01', createdAt: '2024-01-01' }],
      error: null,
    });
    ...
    expect(clientMock.from).toHaveBeenCalledWith('swole-tracker_workout_session');
    expect(clientMock.chain.eq).toHaveBeenCalledWith('user_id', 'user_3');
    expect(clientMock.chain.order).toHaveBeenCalledWith('workoutDate', { ascending: false });
  ...
  it('WorkoutOperationsClient.getSessionExercises filters and orders', async () => {
    ...
    expect(clientMock.from).toHaveBeenCalledWith('swole-tracker_session_exercise');
    expect(clientMock.chain.eq).toHaveBeenCalledWith('sessionId', 99);
    expect(clientMock.chain.order).toHaveBeenCalledWith('createdAt', { ascending: true });
  ...
  it('WorkoutOperationsServer.getRecentWorkouts uses server client factory', async () => {
    const clientMock = makeSupabaseClient();
    clientMock.setResult({
      data: [{ id: 2, templateId: 1, workoutDate: '2024-01-01', createdAt: '2024-01-01' }],
      error: null,
    });
    ...
    const { WorkoutOperationsServer } = await import('~/lib/workout-operations');
  ...
  it('useWorkoutOperations returns a client instance; getServerWorkoutOperations returns server instance', async () => {
    ...
    const { useWorkoutOperations, getServerWorkoutOperations, WorkoutOperationsClient, WorkoutOperationsServer } =
