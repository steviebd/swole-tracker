...
const warnSpy = vi.fn();
vi.mock('~/lib/logger', () => ({ logger: { error: errorSpy, warn: warnSpy, info: vi.fn() } }));
...
describe('whoop-webhook helpers additional coverage', () => {
  ...
  it('extractWebhookHeaders returns null for exact boundary failures', async () => {
    ...
    headers.set('X-WHOOP-Signature-Timestamp', String(now - (5 * 60 * 1000 + 1))); // 5 mins + 1ms ago
  ...
  it('verifyWhoopWebhook handles invalid base64 signature gracefully', async () => {
    ...
    const result = verifyWhoopWebhook('payload', '%%%invalid_base64%%%', '1234567890');
