...
describe("progressFillVariants", () => {
  it("should be a function for dynamic width", () => {
    const variant = progressFillVariants["visible"] as (
      progress: number,
    ) => any;
...
describe("numberCountVariants", () => {
  ...
  it("should have correct counting state", () => {
    expect((numberCountVariants["counting"] as any).scale).toEqual([
      1, 1.05, 1,
    ]);
  });
});
...
describe("celebrationVariants", () => {
  ...
  it("should have correct celebrate state", () => {
    expect((celebrationVariants["celebrate"] as any).scale).toEqual([
      1, 1.2, 1,
    ]);
    expect((celebrationVariants["celebrate"] as any).rotate).toEqual([
      0, 360, 0,
    ]);
  });
});
...
describe("shimmerVariants", () => {
  it("should have correct animate state", () => {
    ...
    expect((shimmerVariants["animate"] as any).transition?.repeat).toBe(
      Infinity,
    );
