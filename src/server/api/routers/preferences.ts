  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const patch: {
        ...
        predictive_defaults_enabled?: number; // 0/1 in D1
        right_swipe_action?: "collapse_expand" | "none";
        enable_manual_wellness?: number; // 0/1 in D1
      ...
      if (typeof input.predictive_defaults_enabled !== "undefined") {
        patch.predictive_defaults_enabled = input.predictive_defaults_enabled ? 1 : 0;
      }
      ...
      if (typeof input.enable_manual_wellness !== "undefined") {
        patch.enable_manual_wellness = input.enable_manual_wellness ? 1 : 0;
      }
      ...
      if (typeof input.linear_progression_kg !== "undefined") {
        const v = parseFloat(input.linear_progression_kg);
        if (!Number.isNaN(v)) patch.linear_progression_kg = v;
      }
      if (typeof input.percentage_progression !== "undefined") {
        const v = parseFloat(input.percentage_progression);
        if (!Number.isNaN(v)) patch.percentage_progression = v;
      }
      ...
      if (existing) {
        ...
        await ctx.db.insert(userPreferences).values({
          ...
          predictive_defaults_enabled:
            (input.predictive_defaults_enabled ?? false) ? 1 : 0,
          right_swipe_action: input.right_swipe_action ?? "collapse_expand",
          enable_manual_wellness: (input.enable_manual_wellness ?? false) ? 1 : 0,
          progression_type: input.progression_type ?? "adaptive",
          linear_progression_kg: (() => {
            const v = parseFloat(input.linear_progression_kg ?? "2.5");
            return Number.isNaN(v) ? 2.5 : v;
          })(),
          percentage_progression: (() => {
            const v = parseFloat(input.percentage_progression ?? "2.5");
            return Number.isNaN(v) ? 2.5 : v;
          })(),
        });
      }
