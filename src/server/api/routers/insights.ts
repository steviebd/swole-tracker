...
import { workoutSessions, sessionExercises, templateExercises, exerciseLinks } from "~/server/db/schema";
...
function estimate1RM(weight: number | undefined, reps: number | undefined): number | undefined {
  if (!weight || !reps) return undefined;
  return weight * (1 + reps / 30);
}

function compareUnits(weight: number | undefined, unit: "kg" | "lbs", targetUnit: "kg" | "lbs"): number | undefined {
...
export const insightsRouter = createTRPCRouter({
    ...
    .query(async ({ input, ctx }): Promise<ExerciseInsightsResponse> => {
      ...
      if (input.templateExerciseId) {
        const link = await ctx.db.query.exerciseLinks.findFirst({
          where: and(eq(exerciseLinks.templateExerciseId, input.templateExerciseId), eq(exerciseLinks.user_id, ctx.user.id)),
          with: { masterExercise: true },
        });

        if (link) {
          const linked = await ctx.db.query.exerciseLinks.findMany({
            where: and(eq(exerciseLinks.masterExerciseId, link.masterExerciseId), eq(exerciseLinks.user_id, ctx.user.id)),
            with: { templateExercise: true },
          });
          exerciseNamesToSearch = linked.map((l) => l.templateExercise.exerciseName);
          ...
          const te = await ctx.db.query.templateExercises.findFirst({
            where: and(eq(templateExercises.id, input.templateExerciseId), eq(templateExercises.user_id, ctx.user.id)),
          });
      ...
      const recentSessions = await ctx.db.query.workoutSessions.findMany({
        ...
        with: {
          exercises: {
            where: templateExerciseIds && templateExerciseIds.length > 0
      ...
      for (const fs of flat) {
        ...
        if (!prev) {
          bySession.set(fs.sessionId, {
            ...
            bestSet: { weight: weightTarget, reps: fs.reps ?? undefined, unit: input.unit, sets: fs.sets ?? undefined, rpe: fs.rpe ?? undefined },
            est1RM: est,
          });
        } else {
          prev.volume += vol;
          if ((weightTarget ?? -Infinity) > (prev.bestWeight ?? -Infinity)) {
            ...
            prev.bestSet = { weight: weightTarget, reps: fs.reps ?? undefined, unit: input.unit, sets: fs.sets ?? undefined, rpe: fs.rpe ?? undefined };
          }
      ...
      if (recentWeights.length >= 2) {
        ...
        if (trendUp) {
          if (input.unit === "kg") {
            ...
            recommendation = { type: "weight", nextWeight: Math.round((last + inc) * 2) / 2, rationale: "Upward trend; suggest ~2.5% increase", unit: input.unit };
          } else {
            const inc = 5; // lbs plate jump
            recommendation = { type: "weight", nextWeight: last + inc, rationale: "Upward trend; suggest +5 lb", unit: input.unit };
          }
        } else {
          // If not trending up, suggest reps progression by +1 if reps exists and <= 10
          recommendation = { type: "reps", nextReps: 1, rationale: "Plateau detected; try adding a rep", unit: input.unit };
        }
      }
      ...
      if (rpeValues.length >= 3) {
        ...
        if (avgRpe <= 6) suggestions.push({ kind: "rpe", message: "Average RPE is low (≤6). Consider increasing weight or reps to target RPE 7–9." });
      }
      if (restValues.length >= 3) {
        const avgRest = restValues.reduce((a, b) => a + b, 0) / restValues.length;
        if (avgRest < 60) suggestions.push({ kind: "rest", message: "Average rest is short (<60s). Consider increasing rest between sets." });
        if (avgRest > 180) suggestions.push({ kind: "rest", message: "Average rest is long (>180s). Consider reducing rest to maintain intensity." });
      }
      if (sparkline.length >= 3) {
        const volTrendUp = sparkline[sparkline.length - 1]!.volume >= sparkline[0]!.volume;
        if (!volTrendUp) suggestions.push({ kind: "volume", message: "Training volume trending flat/down. Consider progressive overload plan." });
      }
  ...
  getSessionInsights: protectedProcedure
    ...
    .query(async ({ input, ctx }): Promise<SessionInsightsResponse> => {
      ...
      for (const ex of session.exercises) {
        ...
        const weight = compareUnits(toNumber(ex.weight), (ex.unit as "kg" | "lbs") ?? "kg", input.unit);
        const reps = ex.reps ?? 0;
        const vol = (weight ?? 0) * reps;

        const entry = byExercise.get(key) ?? { volume: 0 };
        entry.volume += vol;
        if (!entry.bestSet || (weight ?? -Infinity) > (entry.bestSet.weight ?? -Infinity)) {
          entry.bestSet = { weight, reps, unit: input.unit };
        }
        byExercise.set(key, entry);
      }

      const totalVolume = Math.round(Array.from(byExercise.values()).reduce((a, b) => a + b.volume, 0) * 100) / 100;
      const bestSets = Array.from(byExercise.entries()).map(([exerciseName, v]) => ({ exerciseName, ...v }));
  ...
  exportWorkoutsCSV: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return { filename: "workouts_export.csv", mimeType: "text/csv", content: rows.join("\n") };
    }),
});
