...
export const suggestionsRouter = createTRPCRouter({
  ...
  getAnalytics: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const modifiedCount = interactions.filter(
        (i) => i.action === "modified",
      ).length;

      const acceptanceRate =
        totalInteractions > 0 ? (acceptedCount / totalInteractions) * 100 : 0;

      // Group by exercise to find which exercises have highest acceptance
      const exerciseStats = interactions.reduce(
        (stats, interaction) => {
          ...
          if (interaction.action === "accepted")
            stats[interaction.exerciseName]!.accepted++;
          if (interaction.action === "rejected")
            stats[interaction.exerciseName]!.rejected++;
          if (interaction.action === "modified")
            stats[interaction.exerciseName]!.modified++;
