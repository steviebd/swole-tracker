...
export const userMigration = createTable(
  ...
  (d) => ({
    id: d.integer().primaryKey().generatedByDefaultAsIdentity(),
    supabase_user_id: d.varchar({ length: 256 }).unique().notNull(),
    workos_user_id: d.varchar({ length: 256 }).unique().notNull(),
    migration_status: d.varchar({ length: 20 }).notNull().default("pending"), // 'pending', 'completed', 'failed'
    createdAt: d
      .timestamp({ withTimezone: true })
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
    migrated_at: d.timestamp({ withTimezone: true }),
  }),
  (t) => [
