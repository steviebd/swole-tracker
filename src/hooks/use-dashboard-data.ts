...
import { buildWorkoutSummary, type RecentWorkout } from "~/lib/workout-metrics";
...
export function useDashboardData(
  ...
  const {
    ...
    useRecentWorkoutsQuery: useRecentWorkoutsQueryImpl = api.workouts.getRecent
      .useQuery,
  } = dependencies;
  ...
  const weeklyGoals = useProgressGoalsImpl({
    timeRange: "week",
    ...(customTargets?.weekly && { customTargets: customTargets.weekly }),
  });

  const monthlyGoals = useProgressGoalsImpl({
    timeRange: "month",
    ...(customTargets?.monthly && { customTargets: customTargets.monthly }),
  });
  ...
  const helpers = useMemo(
    () => ({
      ...
      shouldShowCelebration: () => {
        ...
        const weeklyExceeded = weeklyGoals.goals.some(
          (goal) => goal.status === "exceeded" || goal.status === "perfect",
        );
